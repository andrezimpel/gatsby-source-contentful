  ...
  const contentfulClientOptions = {
    ...
    responseLogger: response => {
      ...
      if (response.isAxiosError) {
        ...
        reporter.verbose(`${response.config.method} /${response.config.url}: ${response.response.status} ${response.response.statusText} (${createMetadataLog(response.response)})`);
        let errorMessage = `${response.response.status} ${response.response.statusText}`;

        if ((_response$response = response.response) !== null && _response$response !== void 0 && (_response$response$da = _response$response.data) !== null && _response$response$da !== void 0 && _response$response$da.message) {
          errorMessage += `\n\n${response.response.data.message}`;
        }

        const contentfulApiError = new Error(errorMessage); // Special response naming to ensure the error object is not touched by
      ...
      reporter.verbose(`${response.config.method} /${response.config.url}: ${response.status} ${response.statusText} (${createMetadataLog(response)})`);
    }
  };
...
${formatPluginOptionsForCLI(pluginConfig.getOriginalPluginOptions(), errors)}`
  ...
  try {
    syncProgress = reporter.createProgress(`Contentful: ${syncToken ? `Sync changed items` : `Sync all items`}`, pageLimit, 0);
    syncProgress.start();
