...
exports.buildEntryList = ({
  ...
  const map = new Map(contentTypeItems.map(contentType => [contentType.sys.id, []])); // Now fill the buckets. Ignore entries for which there exists no bucket. (Not sure if that ever happens)
...
exports.createNodesForContentType = ({
  ...
  locales.forEach(locale => {
    ...
    const entryNodes = entries.map(entryItem => {
      var _existingNode$interna;

      const entryNodeId = mId(space.sys.id, entryItem.sys.id, entryItem.sys.type);
      const existingNode = getNode(entryNodeId);

      if ((existingNode === null || existingNode === void 0 ? void 0 : (_existingNode$interna = existingNode.internal) === null || _existingNode$interna === void 0 ? void 0 : _existingNode$interna.contentDigest) === entryItem.sys.updatedAt) {
      ...
      Object.keys(entryItemFields).forEach(entryItemFieldKey => {
        ...
        if (fieldType === `Text`) {
          ...
          if ((existingNode === null || existingNode === void 0 ? void 0 : (_existingNode$interna2 = existingNode.internal) === null || _existingNode$interna2 === void 0 ? void 0 : _existingNode$interna2.contentDigest) !== entryItem.sys.updatedAt) {
            const textNode = prepareTextNode(textNodeId, entryNode, entryItemFieldKey, entryItemFields[entryItemFieldKey], createNodeId);
          ...
          if ((existingNode === null || existingNode === void 0 ? void 0 : (_existingNode$interna3 = existingNode.internal) === null || _existingNode$interna3 === void 0 ? void 0 : _existingNode$interna3.contentDigest) !== entryItem.sys.updatedAt) {
            const jsonNode = prepareJSONNode(jsonNodeId, entryNode, entryItemFieldKey, entryItemFields[entryItemFieldKey], createNodeId);
          ...
          entryItemFields[entryItemFieldKey].forEach((obj, i) => {
            ...
            if ((existingNode === null || existingNode === void 0 ? void 0 : (_existingNode$interna4 = existingNode.internal) === null || _existingNode$interna4 === void 0 ? void 0 : _existingNode$interna4.contentDigest) !== entryItem.sys.updatedAt) {
              const jsonNode = prepareJSONNode(jsonNodeId, entryNode, entryItemFieldKey, obj, createNodeId, i);
              childrenNodes.push(jsonNode);
            }
      ...
      return entryNode;
    }).filter(Boolean); // Create a node for each content type
