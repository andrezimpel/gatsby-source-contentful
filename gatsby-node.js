...
const validateContentfulAccess = async pluginOptions => {
  ...
  await fetch(`https://${pluginOptions.host}/spaces/${pluginOptions.spaceId}/environments/${pluginOptions.environment}/content_types`, {
    ...
    if (!ok) {
      let errorMessage = `Cannot access Contentful space "${maskText(pluginOptions.spaceId)}" on environment "${pluginOptions.environment} with access token "${maskText(pluginOptions.accessToken)}". Make sure to double check them!`;
...
List of locales and their codes can be found in Contentful app -> Settings -> Locales`).default(() => () => true),
  forceFullSync: Joi.boolean().description(`Prevents the use of sync tokens when accessing the Contentful API.`).default(false),
  pageLimit: Joi.number().integer().description(`Number of entries to retrieve from Contentful at a time. Due to some technical limitations, the response payload should not be greater than 7MB when pulling content from Contentful. If you encounter this issue you can set this param to a lower number than 100, e.g 50.`).default(100),
...
exports.sourceNodes = async ({
  ...
  if (process.env.GATSBY_CONTENTFUL_EXPERIMENTAL_FORCE_CACHE) {
    reporter.info(`GATSBY_CONTENTFUL_EXPERIMENTAL_FORCE_CACHE: Storing/loading remote data through \`` + process.env.GATSBY_CONTENTFUL_EXPERIMENTAL_FORCE_CACHE + `\` so Remote changes CAN NOT be detected!`);
  }

  const forceCache = await fs.exists(process.env.GATSBY_CONTENTFUL_EXPERIMENTAL_FORCE_CACHE);
  ...
  if (forceCache) {
    ...
    reporter.info(`GATSBY_CONTENTFUL_EXPERIMENTAL_FORCE_CACHE was set. Skipping remote fetch, using data stored in \`${process.env.GATSBY_CONTENTFUL_EXPERIMENTAL_FORCE_CACHE}\``);
    ({
      currentSyncData,
      contentTypeItems,
      defaultLocale,
      locales,
      space
    } = v8.deserialize(fs.readFileSync(process.env.GATSBY_CONTENTFUL_EXPERIMENTAL_FORCE_CACHE)));
  } else {
    ...
    if (process.env.GATSBY_CONTENTFUL_EXPERIMENTAL_FORCE_CACHE) {
      ...
      fs.writeFileSync(process.env.GATSBY_CONTENTFUL_EXPERIMENTAL_FORCE_CACHE, v8.serialize({
        currentSyncData,
        contentTypeItems,
        defaultLocale,
        locales,
...
`);
  ...
  const gqlTypes = contentTypeItems.map(contentTypeItem => schema.buildObjectType({
    name: _.upperFirst(_.camelCase(`Contentful ${pluginConfig.get(`useNameForId`) ? contentTypeItem.name : contentTypeItem.sys.id}`)),
    fields: {
      contentful_id: {
        type: `String!`
      },
  ...
  processingActivity.start(); // Create a map of up to date entries and assets
  ...
  const mergedSyncData = {
    entries: mergeSyncData(previousSyncData.entries, currentSyncData.entries, deletedSet),
    assets: mergeSyncData(previousSyncData.assets, currentSyncData.assets, deletedSet)
  }; // Store a raw and unresolved copy of the data for caching

  const mergedSyncDataRaw = _.cloneDeep(mergedSyncData); // Use the JS-SDK to resolve the entries and assets
  ...
  reporter.info(`Deleted assets ${currentSyncData.deletedAssets.length}`); // Update syncToken

  const nextSyncToken = currentSyncData.nextSyncToken;
  await Promise.all([cache.set(CACHE_SYNC_DATA, mergedSyncDataRaw), cache.set(CACHE_SYNC_TOKEN, nextSyncToken)]);
